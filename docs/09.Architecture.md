...
|------|-------------|
| `/` | Main dashboard |
| `/setup` | First-run setup (Welcome, Hardware, Station, Layout, Tips) |
| `/layout` | Layout configurator; optional query `tour=1` shows the intro tour until `layout_editor_tour_seen` is true |
...
| POST | `/api/layout-editor-tour-seen` | Set `layout_editor_tour_seen` so the layout intro tour is not shown again |
| POST | `/api/config/resolve-module-conflict` | Apply chosen setting across conflicting module instances (layout / map overlay) |
