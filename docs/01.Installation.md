...
| **2. GitHub + installer** | Download the project from GitHub, extract it, then run the installer from the `installers` folder. |
...
1. Download the project: [GitHub main branch (ZIP)](https://github.com/pomtom44/GlanceRF_Dev/archive/refs/heads/main.zip)
2. Extract the ZIP to a folder of your choice
3. Run the installer from the `installers` folder:
...
Config is created at `glancerf_config.json` on first run. Open http://localhost:8080 for setup. After you save setup the first time, the layout editor opens with an optional guided tour (see [User guide](02.UserGuide.md)).
