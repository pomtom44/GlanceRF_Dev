...
_log = get_logger("wwff_cache")

_SPOTS_URL = "https://spots.wwff.co/static/spots.json"
_FETCH_INTERVAL_SEC = 60
_DB_FILENAME = "wwff.db"
_CACHE_DIR = "cache"
...
def _get_wwff_settings_value(key: str, default: float) -> float:
...
def _get_cache_history_hours() -> float:
    """Get cache history (hours past) from ota_programs module settings."""
    return _get_wwff_settings_value("cache_hours_past", _DEFAULT_CACHE_HOURS)
...
def _purge_old_records(conn: sqlite3.Connection) -> None:
    ...
    try:
        ...
        if n_spots:
            conn.commit()
            _log.debug("WWFF cache: purged %d spots", n_spots)
    except sqlite3.Error as e:
        _log.debug("WWFF cache purge error: %s", e)
...
def _fetch_json(url: str) -> list[dict[str, Any]]:
    try:
        ...
        _log.debug("WWFF fetch %s failed: %s", url, e)
    return []


def _run_wwff_cache_thread() -> None:
            ...
            if spots:
                ...
                for s in spots:
                    ...
                    try:
                        ...
                        pass
                conn.commit()
                _log.debug("WWFF cache: stored %d spots", len(spots))
        except Exception as e:
            _log.debug("WWFF cache error: %s", e)
        finally:
            try:
                if conn is not None:
                    conn.close()
...
def start_wwff_cache() -> None:
    ...
    if _thread is not None and _thread.is_alive():
        return
    db_path = _get_cache_db_path()
    _thread = threading.Thread(target=_run_wwff_cache_thread, daemon=True)
    _thread.start()
    _log.debug("WWFF cache started: %s", db_path)
...
def get_cached_spots(
    ...
    try:
        ...
        _log.debug("WWFF cache read spots failed: %s", e)
        return []
